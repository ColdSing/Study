...
<HTML>
		...
		<script type="text/javascript">
			window.onload=function(){
				...
				for (var i = 0; i < select1.options.length; i++){  
			        if (select1.options[i].value == "${product.cid}"){  
			            select1.options[i].selected = true;  
			            break;  
			        }  
			    } 
				var select2 = document.getElementById("selectHot");
				for (var i = 0; i < select2.options.length; i++){  
			        if (select2.options[i].value == "${product.is_hot}"){  
	...
	<body>
		<!--  -->
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/adminProductDoEdit" method="post" ">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						...
						<select name="is_hot" id="selectHot">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<select name="cid" id="selectCategory">
